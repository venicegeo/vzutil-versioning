...
func TestClusterHealth(t *testing.T) {
	...
	// Get cluster health
	res, err := NewClusterHealthService(client).Index(testIndexName).Level("shards").Pretty(true).Do(context.TODO())
	if err != nil {
		t.Fatal(err)
	}
	if res == nil {
...
func TestClusterHealthWaitForStatus(t *testing.T) {
	...
	health, err := NewClusterHealthService(client).Do(context.TODO())
	if err != nil {
		t.Fatal(err)
	}
	...
	health, err = NewClusterHealthService(client).Index("no-such-index").WaitForStatus("yellow").Timeout("1s").Do(context.TODO())
	if err == nil {
	...
	// Cluster wide health
	health, err = NewClusterHealthService(client).WaitForGreenStatus().Timeout("10s").Do(context.TODO())
	if err != nil {
		t.Fatalf("expected no error; got: %v", err)
	}
